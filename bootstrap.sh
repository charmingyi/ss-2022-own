...
for command in git python3; do
    command -v "$command" >/dev/null 2>&1 || { printf '[错误] 需要 %s。\n' "$command" >&2; exit 1; }
done
if [[ "$MODE" == release ]]; then
    for command in curl sha256sum; do
        command -v "$command" >/dev/null 2>&1 || { printf '[错误] 预编译安装需要 %s。\n' "$command" >&2; exit 1; }
    done
    [[ "$RELEASE_SHA256_AMD64_GLIBC" != "__RELEASE_SHA256_TBD__" ]] || {
        printf '%s\n' '[错误] 当前仓库的 Release 哈希尚未配置，请更新到正式发布提交。' >&2
        exit 1
    }
fi
...
if [[ "$MODE" == release ]]; then
    ...
    archive="$TMP_DIR/$RELEASE_ASSET_AMD64_GLIBC"
    release_url="https://github.com/charmingyi/ss-2022-own/releases/download/${RELEASE_TAG}/${RELEASE_ASSET_AMD64_GLIBC}"
    printf '[bootstrap] 下载并校验预编译核心：%s\n' "$RELEASE_TAG"
    curl --fail --silent --show-error --location --proto '=https' --tlsv1.2 \
        --retry 3 --connect-timeout 15 --max-time 600 "$release_url" -o "$archive"
    printf '%s  %s\n' "$RELEASE_SHA256_AMD64_GLIBC" "$archive" | sha256sum -c -
    package_root=$(python3 - "$archive" "$TMP_DIR/package" "$release_arch" <<'PY'
import hashlib
import json
import pathlib
import posixpath
...
archive, destination, expected_arch = sys.argv[1:]
...
if manifest.get("target", {}).get("libc") != "glibc":
